...
import dev.gmarques.compras.presenter.frags_activities.lista_de_compras.FragListaDeCompras
...
class ProdutoAdapter(
    ...
    private val itens: ArrayList<ProdutoEntidade> = ArrayList()
     ...
     *
     */
    fun atualizarColecao(novosItens: ArrayList<ProdutoEntidade>) {
    ...
    inner class ViewHolder(
        ...
        fun bind(produtoEntidade: ProdutoEntidade, indice: Int) {

            alternarMenu(false)

            bindingView.tvNome.text = produtoEntidade.nome
            bindingView.tvPreco.text = produtoEntidade.preco.emMoeda()
            bindingView.tvInfo.text = produtoEntidade.detalhes
            bindingView.tvQtd.text =
                String.format(App.get.applicationContext.getString(R.string.un), produtoEntidade.quantidade)
            bindingView.tvPrecoTotal.text = produtoEntidade.valorTotal().emMoeda()
            bindingView.cbComprado.isChecked = produtoEntidade.comprado

            aplicarEstilo(produtoEntidade)
            ...
            bindingView.cbComprado.setOnClickListener {
                callback.produtoComprado(produtoEntidade, bindingView.cbComprado.isChecked)
            }

            bindingView.fabEditar.setOnClickListener {
                alternarMenu(false)
                callback.editarProduto(produtoEntidade)
            }

            bindingView.fabRemover.setOnClickListener {
                alternarMenu(false)
                callback.produtoRemovido(produtoEntidade)
            }

            bindingView.tvQtd.setOnClickListener {
                callback.quantidadeEditada(produtoEntidade)
            }

            bindingView.tvPreco.setOnClickListener {
                callback.precoEditado(produtoEntidade)
            }
        }
        ...
        private fun aplicarEstilo(produtoEntidade: ProdutoEntidade) = with(produtoEntidade.comprado) {
            bindingView.tvNome.riscarTexto(this)
            bindingView.card.setCardBackgroundColor(if (this) corComprado!! else corNormal!!)
            if (this) bindingView.tvNome.text = "<i>${produtoEntidade.nome}</i>".formatarHtml() /*italico*/
            else bindingView.tvNome.text = produtoEntidade.nome
        }
