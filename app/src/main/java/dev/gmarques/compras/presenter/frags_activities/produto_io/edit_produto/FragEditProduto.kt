package dev.gmarques.compras.presenter.frags_activities.produto_io.edit_produto
...
import dev.gmarques.compras.presenter.frags_activities.categoria_io.AddCategoriaDialog
import dev.gmarques.compras.presenter.frags_activities.produto_io.CategoriaAdapter
import dev.gmarques.compras.presenter.frags_activities.produto_io.CategoriaAdapterCallback
...
class FragEditProduto : Fragment(), CategoriaAdapterCallback {
    ...
    private fun carregarArgumentos() {
        ...
        viewModel.produtoEntidade = args.produto.clonar()
        viewModel.produtoEntidadeOriginal = args.produto
    }
    ...
    private fun mostrarDadosDoItemNaUi() = lifecycleScope.launch {
        ...
        binding.edtNome.setText(viewModel.produtoEntidade.nome)
        binding.edtValor.setText(viewModel.produtoEntidade.preco.toString())
        binding.edtQtd.setText(viewModel.produtoEntidade.quantidade.toString())
        binding.edtDetalhes.setText(viewModel.produtoEntidade.detalhes)
        binding.toolbar.title = String.format(getString(R.string.Editar_x), viewModel.produtoEntidade.nome)
    ...
    private suspend fun validarEntradasECriarObjeto(): Boolean {
        ...
        if (nomeInvalido(nome)) {
            ...
            viewModel.produtoEntidade.nome = nome
            viewModel.produtoEntidade.preco = preco.toFloat()
            viewModel.produtoEntidade.quantidade = qtd.toInt()
            viewModel.produtoEntidade.detalhes = binding.edtDetalhes.text.toString()
            viewModel.produtoEntidade.categoriaId = viewModel.categoriaSelecionada.value!!.id
     ...
     */
    override fun categoriaSelecionada(categoria: CategoriaEntidade) =
    ...
    private fun categoriaAdicionada(categoria: CategoriaEntidade) {
    ...
    private suspend fun produtoRepetido(nome: String): Boolean {
        return if (viewModel.produtoEntidade.nome == viewModel.produtoEntidadeOriginal.nome) false
    ...
    private fun fecharFragmento() {
        setFragmentResult("produtoAtualizado",
            bundleOf(
                "produto" to viewModel.produtoEntidade,
                "produtoOriginal" to viewModel.produtoEntidadeOriginal))
