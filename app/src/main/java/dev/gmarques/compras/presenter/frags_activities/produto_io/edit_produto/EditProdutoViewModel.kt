...
class EditProdutoViewModel : ViewModel() {
    lateinit var produtoEntidade: ProdutoEntidade
    /**Não deve sofrer qualquer tipo de alteraçao*/
    lateinit var produtoEntidadeOriginal: ProdutoEntidade

    private val _categoriaEntidadeSelecionada: MutableLiveData<CategoriaEntidade> = MutableLiveData()
    val categoriaSelecionada get() = _categoriaEntidadeSelecionada

    suspend fun produtoJaExisteNaLista(nome: String) =
        ItemRepo.getItensNaListaPorNome(nome, this.produtoEntidade.listaId).isNotEmpty()
    ...
    fun definirCategoriaSelecionada(categoria: CategoriaEntidade) {
        _categoriaEntidadeSelecionada.value = categoria
    }

    suspend fun receberCategoriaDoItem() = CategoriaRepo.getCategoriaPorId(this.produtoEntidade.categoriaId)
